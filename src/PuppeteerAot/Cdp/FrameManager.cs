...
namespace Puppeteer.Cdp
{
    public class FrameManager : IDisposable, IAsyncDisposable, IFrameProvider
    {
        ...
        public event EventHandler<FrameEventArgs> FrameSwapped;

        public event EventHandler<FrameNavigatedEventArgs> FrameNavigated;

        public event EventHandler<FrameEventArgs> FrameNavigatedWithinDocument;
        ...
        public FrameTree FrameTree { get; } = new();

        public Frame MainFrame => FrameTree.MainFrame;
        ...
        public void OnAttachedToTarget(TargetChangedArgs e)
        {
        ...
        public ExecutionContext GetExecutionContextById(int contextId, CDPSession session)
        {
            _contextIdToContext.TryGetValue($"{session.Id}:{contextId}", out var context);
            return context;
        }

        public DeviceRequestPromptManager GetDeviceRequestPromptManager(CDPSession client)
            => _deviceRequestPromptManagerMap.GetOrAdd(client, _ => new DeviceRequestPromptManager(client, TimeoutSettings));

        public Frame[] GetFrames() => FrameTree.Frames;

        public async Task InitializeAsync(CDPSession client)
        {
            try
            {
        ...
        public async Task SwapFrameTreeAsync(CDPSession client)
        {
        ...
        public Task RegisterSpeculativeSessionAsync(CDPSession client)
                ...
                try
                {
                    ...
                    switch (e.MessageID)
                    {
                        case "Page.frameAttached":
                            OnFrameAttached(sender as CDPSession, e.MessageData.ToObject<PageFrameAttachedResponse>(true));
                            break;
