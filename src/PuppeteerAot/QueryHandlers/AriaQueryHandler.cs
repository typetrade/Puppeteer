// <copyright file="AriaQueryHandler.cs" company="PlaceholderCompany">
...
namespace Puppeteer.QueryHandlers
{
    public class AriaQueryHandler : QueryHandler
    {
        ...
        public override async IAsyncEnumerable<IElementHandle> QueryAllAsync(IElementHandle element, string selector)
        {
        ...
        public override async Task<IElementHandle> QueryOneAsync(IElementHandle element, string selector)
        {
        ...
        private static async Task<IEnumerable<AXTreeNode>> QueryAXTreeAsync(CDPSession client, IElementHandle element, string accessibleName, string role)
        {
            ...
            {
                return node?.Role?.Value.GetString() is not null &&
                        !_nonElementNodeRoles.Contains(node.Role.Value.GetString());
            });
        }
