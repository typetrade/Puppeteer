...
namespace Puppeteer
{
    ...
    public abstract class LauncherBase : IDisposable
    {
        ...
        public TaskCompletionSource<bool> ExitCompletionSource { get; } = new(TaskCreationOptions.RunContinuationsAsynchronously);

        public TaskCompletionSource<string> StartCompletionSource { get; } = new(TaskCreationOptions.RunContinuationsAsynchronously);

        public LaunchOptions Options { get; }

        public TempDirectory TempUserDataDir { get; set; }

        /// <summary>
        /// Gets Base process current state.
        /// </summary>
        public State CurrentState => _stateManager.CurrentState;
